...
[medium article](https://medium.com/@neospcc/epicchain-privatenet-auto-import-of-a-smart-contract-dbf2b9220ad2). 
...
```
epicchain-GO-VM > help
NAME:
   VM CLI - Official VM CLI for epicchain-Go
...
import (
	"github.com/nspcc-dev/epicchain-go/pkg/interop/runtime"
)
...
```
epicchain-GO-VM > loadgo 1-print.go
...
You can find more information on how to use the CLI [here](https://github.com/nspcc-dev/epicchain-go/blob/master/docs/cli.md)
...
Here is the document where you can find supported [RPC calls](https://github.com/nspcc-dev/epicchain-go/blob/master/docs/rpc.md).
...
#### Virtual machine
Virtual machine runs compiled byte code. NeoVM is a [stack-based virtual machine](https://docs.epic-chain.org/docs/en-us/basic/technology/neovm.html). It has 2 stacks for performing computation.
...
EpicChain has a nice monitor where you can find particular node running in the blockchain network.
[EpicChain Monitor](http://monitor.cityofzion.io/)

## Workshop. Content
The workshop contains a guide, examples and tips on EpicChain smart contracts development
and dApps development for EpicChain ecosystem using development kit offered by EpicChainG.
The workshop is divided into multiple parts:
1. [**Preparation.**](#workshop-preparation) Learn how to run a EpicChain private network,
  transfer some funds from multi-signature account to a simple signature account
  using EpicChainG CLI and check the balance via JSON RPC call.
2. [**Part 1.**](#workshop-part-1) Compile, inspect, deploy and invoke simple
  `Hello, world!` smart contract written in Go.
3. [**Part 2.**](#workshop-part-2) Investigate EpicChain JSON RPC protocol and EpicChainG CLI
   utilities to retrieve information from EpicChain RPC nodes. Get acquainted with the
...
6. [**Part 5.**](#workshop-part-5) Learn how to develop simple decentralized application
   for the EpicChain ecosystem using tools provided by EpicChainG.
 
## Workshop. Preparation
In this part we will setup the environment: run private network, connect epicchain-go node to it and transfer some initial GAS to our basic account
...
#### Step 1
If you already have epicchain-go or go smart-contracts, please, update go modules in order to be up-to-date with the current interop API changes.
If not, download epicchain-go and build it (master branch):
```
$ git clone https://github.com/nspcc-dev/epicchain-go.git
$ cd epicchain-go
$ make build 
```

#### Step 2
To run EpicChainG-based 4-node private network use this commands:
...
```
=> Bootup environment
Creating network "epicchain_go_network" with the default driver
Creating volume "docker_volume_chain" with local driver
Creating epicchain_go_node_four  ... done
Creating epicchain_go_node_two   ... done
Creating epicchain_go_node_one   ... done
Creating epicchain_go_node_three ... done
```
...
#### Step 3
Start epicchain-go node which will connect to previously started privatenet:
```
$ ./bin/epicchain-go node --privnet
```
...
/epicchain-GO:0.91.1-pre-657-gc13d6ecc/

2020-12-17T14:51:53.204+0300	INFO	new peer connected	{"addr": "127.0.0.1:20333", "peerCount": 1}
2020-12-17T14:51:53.206+0300	INFO	started protocol	{"addr": "127.0.0.1:20333", "userAgent": "/epicchain-GO:0.91.1-pre-657-gc13d6ecc/", "startHeight": 0, "id": 3172166887}
...
2020-12-17T14:51:56.205+0300	INFO	started protocol	{"addr": "127.0.0.1:20336", "userAgent": "/epicchain-GO:0.91.1-pre-657-gc13d6ecc/", "startHeight": 0, "id": 90708676}
2020-12-17T14:51:56.206+0300	WARN	peer disconnected	{"addr": "127.0.0.1:20333", "reason": "already connected", "peerCount": 4}
2020-12-17T14:51:56.206+0300	INFO	started protocol	{"addr": "127.0.0.1:20334", "userAgent": "/epicchain-GO:0.91.1-pre-657-gc13d6ecc/", "startHeight": 0, "id": 410946741}
2020-12-17T14:51:56.207+0300	INFO	started protocol	{"addr": "127.0.0.1:20335", "userAgent": "/epicchain-GO:0.91.1-pre-657-gc13d6ecc/", "startHeight": 0, "id": 4085957952}
...
1. Create NEP-17 transfer transaction:
    ```
        $ ./bin/epicchain-go wallet nep17 transfer -w .docker/wallets/wallet1.json --out my_tx.json -r http://localhost:20331 --from NVTiAjNgagDkTr5HTzDmQP9kPwPHN5BgVq --to NbrUYaZgyhSkNoRo9ugRyEMdUZxrhkNaWB --token GAS --amount 29999999
    ``` 
    Where
    - `./bin/epicchain-go` runs epicchain-go
    - `wallet nep17 transfer` - command with arguments in [epicchain-go](https://github.com/nspcc-dev/epicchain-go/blob/master/cli/wallet/nep17.go#L108)
    - `-w .docker/wallets/wallet1.json` - path to the [wallet](https://github.com/nspcc-dev/epicchain-go/blob/master/.docker/wallets/wallet1.json) for the first node in the private network
    ...
    - `--to NbrUYaZgyhSkNoRo9ugRyEMdUZxrhkNaWB` - our account from the [wallet](https://github.com/nspcc-dev/epicchain-go-sc-wrkshp/blob/master/my_wallet.json)
...
2. Sign the created transaction using the second node address:
    ...
    ```
    $ ./bin/epicchain-go wallet sign -w .docker/wallets/wallet2.json --in my_tx.json --out my_tx2.json --address NVTiAjNgagDkTr5HTzDmQP9kPwPHN5BgVq
    ```
    Where
    - `-w .docker/wallets/wallet2.json` - path to the [wallet](https://github.com/nspcc-dev/epicchain-go/blob/master/.docker/wallets/wallet2.json) for the second node in private network
...
3. Sign the transaction using the third node address and push it to the chain:
    ```
    $ ./bin/epicchain-go wallet sign -w ./.docker/wallets/wallet3.json --in my_tx2.json --out my_tx3.json --address NVTiAjNgagDkTr5HTzDmQP9kPwPHN5BgVq -r http://localhost:20331
    ```
...
[1-print
directory](https://github.com/nspcc-dev/epicchain-go-sc-wrkshp/tree/master/1-print). The
...
Contract configuration is available in the same directory, [1-print.yml](https://github.com/nspcc-dev/epicchain-go-sc-wrkshp/blob/master/1-print/1-print.yml).

#### Step 2
Compile "Hello World" smart contract:
```
$ ./bin/epicchain-go contract compile -i 1-print/1-print.go -c 1-print/1-print.yml -m 1-print/1-print.manifest.json
```
Where
- `./bin/epicchain-go` runs epicchain-go
- `contract compile` command with arguments in [epicchain-go](https://github.com/nspcc-dev/epicchain-go/blob/master/cli/smartcontract/smart_contract.go#L105)
...
```
$ ./bin/epicchain-go contract inspect -i 1-print/1-print.nef
```

#### Step 3
Deploy smart contract to the previously setup network:
```
$ ./bin/epicchain-go contract deploy -i 1-print/1-print.nef -manifest 1-print/1-print.manifest.json -r http://localhost:20331 -w my_wallet.json
```
...
Invoke contract.
```
$ ./bin/epicchain-go contract invokefunction -r http://localhost:20331 -w my_wallet.json bfad19135422aaddf2fc86f86ec5d4b1371e8e93 main
```
...
Each epicchain-go node provides an API interface for obtaining blockchain data from it.
The interface is provided via `JSON-RPC`, and the underlying protocol uses HTTP for communication.

Full `EpicChain JSON-RPC 3.0 API` described [here](https://docs.epic-chain.org/v3/docs/en-us/reference/rpc/latest-version/api.html).

RPC-server of started in step #3 epicchain-go node is available on `localhost:20331`, so let's try to perform several RPC calls.

#### GetRawTransaction
[GetRawTransaction](https://docs.epic-chain.org/v3/docs/en-us/reference/rpc/latest-version/api/getrawtransaction.html) returns 
...
#### GetApplicationLog
[GetApplicationLog](https://docs.epic-chain.org/v3/docs/en-us/reference/rpc/latest-version/api/getapplicationlog.html) returns the contract log based on the specified transaction id.
...
List of supported by epicchain-go node RPC commands you can find [here](https://github.com/nspcc-dev/epicchain-go/blob/master/docs/rpc.md#supported-methods).

#### Utilities

epicchain-go CLI provides `query tx` utility to check the transaction state. It uses
...
to the chain:
```
./bin/epicchain-go query tx 60fbf79b06714e34a3f55e782bf509eedcc602661e49848f5611aaaf2f3442de -r http://localhost:20331 -v
```
where
- `60fbf79b06714e34a3f55e782bf509eedcc602661e49848f5611aaaf2f3442de` - invocation transaction hash from step #7
- `-r http://localhost:20331` - RPC node endpoint
...
Let's take a look at the another smart contract example: [2-storage.go](https://github.com/nspcc-dev/epicchain-go-sc-wrkshp/blob/master/2-storage/2-storage.go).
...
#### Step #1
Compile smart contract [2-storage.go](https://github.com/nspcc-dev/epicchain-go-sc-wrkshp/blob/master/2-storage/2-storage.go):

```
$ ./bin/epicchain-go contract compile -i 2-storage/2-storage.go -c 2-storage/2-storage.yml -m 2-storage/2-storage.manifest.json
```
...
```
$ ./bin/epicchain-go contract deploy -i 2-storage/2-storage.nef -manifest 2-storage/2-storage.manifest.json -r http://localhost:20331 -w my_wallet.json
```
...
Let's check:
```
$ ./bin/epicchain-go contract invokefunction -r http://localhost:20331 -w my_wallet.json ccd533440d0317e9f366c50648d0013540e82741 main
```
... enter the password `qwerty`:
```
Enter account NbrUYaZgyhSkNoRo9ugRyEMdUZxrhkNaWB password >
...
### NEP-17
[NEP-17](https://github.com/epicchain-project/proposals/blob/master/nep-17.mediawiki) is a token standard for the EpicChain blockchain that provides systems with a generalized interaction mechanism for tokenized smart contracts.
The example with implementation of all required by the standard methods you can find in [nep17.go](https://github.com/nspcc-dev/epicchain-go/blob/master/examples/token/nep17/nep17.go)
 
Let's take a view on the example of smart contract with NEP-17: [token.go](https://github.com/nspcc-dev/epicchain-go/blob/master/examples/token/token.go)
...
#### Step #1
To compile [token.go](https://github.com/nspcc-dev/epicchain-go/blob/master/examples/token/token.go)
you can use [configuration](https://github.com/nspcc-dev/epicchain-go/blob/master/examples/token/token.yml).

Compile smart contract:
```
$ ./bin/epicchain-go contract compile -i examples/token/token.go -c examples/token/token.yml -m examples/token/token.manifest.json
```

Deploy smart contract:
```
$ ./bin/epicchain-go contract deploy -i examples/token/token.nef -manifest examples/token/token.manifest.json -r http://localhost:20331 -w my_wallet.json
```
... enter the password `qwerty`:
```
Enter account NbrUYaZgyhSkNoRo9ugRyEMdUZxrhkNaWB password >
...
```
$ ./bin/epicchain-go contract invokefunction -r http://localhost:20331 -w my_wallet.json c36534b6b81621178980438c18796f23a463441a symbol
```                                                                   
Where
...
```
$ ./bin/epicchain-go contract invokefunction -r http://localhost:20331 -w my_wallet.json c36534b6b81621178980438c18796f23a463441a decimals
$ ./bin/epicchain-go contract invokefunction -r http://localhost:20331 -w my_wallet.json c36534b6b81621178980438c18796f23a463441a totalSupply
```
...
```
$ ./bin/epicchain-go contract invokefunction -r http://localhost:20331 -w my_wallet.json c36534b6b81621178980438c18796f23a463441a balanceOf NbrUYaZgyhSkNoRo9ugRyEMdUZxrhkNaWB
```                             
...
```
$ ./bin/epicchain-go contract invokefunction -r http://localhost:20331 -w my_wallet.json c36534b6b81621178980438c18796f23a463441a mint NbrUYaZgyhSkNoRo9ugRyEMdUZxrhkNaWB -- NbrUYaZgyhSkNoRo9ugRyEMdUZxrhkNaWB:CalledByEntry
```
Where:
...
```
$ ./bin/epicchain-go contract invokefunction -r http://localhost:20331 -w my_wallet.json c36534b6b81621178980438c18796f23a463441a transfer NbrUYaZgyhSkNoRo9ugRyEMdUZxrhkNaWB NgzuJWWGVEwFGsRrgzj8knswEYRJrTe7sm 500000000 null -- NbrUYaZgyhSkNoRo9ugRyEMdUZxrhkNaWB:CalledByEntry
```
...
```
$ ./bin/epicchain-go contract invokefunction -r http://localhost:20331 -w my_wallet.json c36534b6b81621178980438c18796f23a463441a balanceOf NgzuJWWGVEwFGsRrgzj8knswEYRJrTe7sm
```
...
## Workshop. Part 4
In this part we'll summarise our knowledge about smart contracts by investigating [4-domain](https://github.com/nspcc-dev/epicchain-go-sc-wrkshp/blob/master/4-domain/4-domain.go) smart contract. This contract
contains code for domain registration, transferring, deletion and getting information about registered domains.

Let’s go!

#### Step #1
Let's take a glance at our [contract](https://github.com/nspcc-dev/epicchain-go-sc-wrkshp/blob/master/4-domain/4-domain.go) and inspect it. The contract takes an action string as the first parameter, which is one of the following:
- `register` checks, whether domain with the specified name already exists. If not, it also adds the pair `[domainName, owner]` to the storage. It requires additional arguments:
   ...
   - `owner` - the 34-digit account address from our [wallet](https://github.com/nspcc-dev/epicchain-go-sc-wrkshp/blob/master/my_wallet.json), which will be used for contract invocation.
...
Compile smart contract [4-domain.go](https://github.com/nspcc-dev/epicchain-go-sc-wrkshp/blob/master/4-domain/4-domain.go) with [configuration](https://github.com/nspcc-dev/epicchain-go-sc-wrkshp/blob/master/4-domain/4-domain.yml)
```
$ ./bin/epicchain-go contract compile -i 4-domain/4-domain.go -c 4-domain/4-domain.yml -m 4-domain/4-domain.manifest.json
```

... and deploy it:
```
$ ./bin/epicchain-go contract deploy -i 4-domain/4-domain.nef --manifest 4-domain/4-domain.manifest.json -r http://localhost:20331 -w my_wallet.json
```
... enter the password `qwerty`:
```
Enter account NbrUYaZgyhSkNoRo9ugRyEMdUZxrhkNaWB password >
...
```
$ ./bin/epicchain-go contract invokefunction -r http://localhost:20331 -w my_wallet.json 9042814f07d65d2b835fa1f07d21c22c6e1cbdf7 register my_first_domain NbrUYaZgyhSkNoRo9ugRyEMdUZxrhkNaWB -- NbrUYaZgyhSkNoRo9ugRyEMdUZxrhkNaWB:CalledByEntry
```
...
```
Also you can see the log message in the console, where you run epicchain-go node:
```
...
```
$ ./bin/epicchain-go contract invokefunction -r http://localhost:20331 -w my_wallet.json 9042814f07d65d2b835fa1f07d21c22c6e1cbdf7 query my_first_domain
```
... the pass `qwerty`:
```
Enter account NbrUYaZgyhSkNoRo9ugRyEMdUZxrhkNaWB password >
...
```
$ ./bin/epicchain-go contract invokefunction -r http://localhost:20331 -w my_wallet.json 9042814f07d65d2b835fa1f07d21c22c6e1cbdf7 transfer my_first_domain NgzuJWWGVEwFGsRrgzj8knswEYRJrTe7sm -- NbrUYaZgyhSkNoRo9ugRyEMdUZxrhkNaWB
```
... the password: `qwerty`
```
Enter account NbrUYaZgyhSkNoRo9ugRyEMdUZxrhkNaWB password >
...
```
$ ./bin/epicchain-go contract invokefunction -r http://localhost:20331 -w my_wallet.json 9042814f07d65d2b835fa1f07d21c22c6e1cbdf7 register my_second_domain NbrUYaZgyhSkNoRo9ugRyEMdUZxrhkNaWB -- NbrUYaZgyhSkNoRo9ugRyEMdUZxrhkNaWB:CalledByEntry
$ ./bin/epicchain-go contract invokefunction -r http://localhost:20331 -w my_wallet.json 9042814f07d65d2b835fa1f07d21c22c6e1cbdf7 delete my_second_domain -- NbrUYaZgyhSkNoRo9ugRyEMdUZxrhkNaWB
```

## Workshop. Part 5

In this part we'll take a look at the example of a decentralized application for the
EpicChain ecosystem written in Go. The example demonstrates how to use EpicChainG RPC Client
...
account from the [wallet](https://github.com/nspcc-dev/epicchain-go-sc-wrkshp/blob/e11949bf5f1dc1ce4e3b6551b6ae22032945c75d/my_wallet.json)
...
* [JSON-RPC client](https://pkg.go.dev/github.com/nspcc-dev/epicchain-go/pkg/rpcclient) creation, initialization and example of simple EpicChain JSON RPC APIs usage.
* EpicChain wallet and account managing with [EpicChainG `wallet` package](https://pkg.go.dev/github.com/nspcc-dev/epicchain-go/pkg/wallet).
* [Extensions](https://github.com/nspcc-dev/epicchain-go/blob/5fc61be5f6c5349d8de8b61967380feee6b51c55/docs/rpc.md#extensions) offered by EpicChainG JSON RPC server and supported by the EpicChainG RPC client.
* EpicChainG JSON RPC server [web-socket extension](https://github.com/nspcc-dev/epicchain-go/blob/5fc61be5f6c5349d8de8b61967380feee6b51c55/docs/rpc.md#websocket-server). [Web-socket JSON RPC Client](https://pkg.go.dev/github.com/nspcc-dev/epicchain-go/pkg/rpcclient#WSClient) creation, initialization and usage example. 
* [`unwrap` helper package](https://pkg.go.dev/github.com/nspcc-dev/epicchain-go/pkg/rpcclient/unwrap) for test invocation results unwrapping.
* EpicChainG JSON RPC [Notification subsystem](https://github.com/nspcc-dev/epicchain-go/blob/5fc61be5f6c5349d8de8b61967380feee6b51c55/docs/notifications.md) usage example. Subscribe for a various set of blockchain node events and receive notifications over web-socket channel.
* EpicChainG `invoker` package usage example. Perform test invocation of smart contract, script or verification method with the powerful [Invoker API](https://pkg.go.dev/github.com/nspcc-dev/epicchain-go/pkg/rpcclient/invoker).
* How to work with witnesses and [witness scopes](https://neospcc.medium.com/thou-shalt-check-their-witnesses-485d2bf8375d).
* [Historic invocations functionality](https://github.com/nspcc-dev/epicchain-go/blob/5fc61be5f6c5349d8de8b61967380feee6b51c55/docs/rpc.md#invokecontractverifyhistoric-invokefunctionhistoric-and-invokescripthistoric-calls) provided by EpicChainG RPC server and client extensions.
* EpicChainG `actor` package usage example. Build, tune, sign, send and await transactions with our flexible [Actor API](https://pkg.go.dev/github.com/nspcc-dev/epicchain-go/pkg/rpcclient/actor).
* Work with a set of NEP-specific and native-specific packages:
  * [`nep17`](https://pkg.go.dev/github.com/nspcc-dev/epicchain-go/pkg/rpcclient/nep17) and [`nep11`](https://pkg.go.dev/github.com/nspcc-dev/epicchain-go/pkg/rpcclient/nep11) packages for NEP-17 or NEP-11 compatible EpicChain token management.
  * Native contracts specific actors: [`gas`](https://pkg.go.dev/github.com/nspcc-dev/epicchain-go/pkg/rpcclient/gas), [`neo`](https://pkg.go.dev/github.com/nspcc-dev/epicchain-go/pkg/rpcclient/neo), [`management`](https://pkg.go.dev/github.com/nspcc-dev/epicchain-go/pkg/rpcclient/management), [`policy`](https://pkg.go.dev/github.com/nspcc-dev/epicchain-go/pkg/rpcclient/policy), [`oracle`](https://pkg.go.dev/github.com/nspcc-dev/epicchain-go/pkg/rpcclient/oracle), [`rolemgmt`](https://pkg.go.dev/github.com/nspcc-dev/epicchain-go/pkg/rpcclient/rolemgmt), [`notary`](https://pkg.go.dev/github.com/nspcc-dev/epicchain-go/pkg/rpcclient/notary) Actor packages.
* Deploy and invoke an [example contract](https://github.com/nspcc-dev/epicchain-go/tree/5fc61be5f6c5349d8de8b61967380feee6b51c55/examples/storage) with the help of native [`ContractManagement` specific Actor package](https://pkg.go.dev/github.com/nspcc-dev/epicchain-go/pkg/rpcclient/management).
* Contract storage iterator API demonstration:
  * Script-based iterator unwrapping via [`CallAndExpandIterator` Invoker API](https://pkg.go.dev/github.com/nspcc-dev/epicchain-go/pkg/rpcclient/invoker#Invoker.CallAndExpandIterator).
  * Session-based iterator traversal via [`TraverseIterator` Invoker API](https://pkg.go.dev/github.com/nspcc-dev/epicchain-go/pkg/rpcclient/invoker#Invoker.TraverseIterator).
* Usage of autogenerated [RPC smart contract binding](https://github.com/nspcc-dev/epicchain-go/blob/5fc61be5f6c5349d8de8b61967380feee6b51c55/docs/compiler.md#generating-rpc-contract-bindings).
...
example, clone the EpicChainG repo to the same folder where the workshop repo is
placed and compile the example Storage contract:
```
$ git clone https://github.com/nspcc-dev/epicchain-go
$ cd epicchain-go
$ make build
$ ./bin/epicchain-go contract compile -i ./examples/storage/storage.go -c ./examples/storage/storage.yml -o examples/storage/storage.nef -m ./examples/storage/storage.manifest.json
```  
...
* [Our basic tutorial on Medium](https://medium.com/@neospcc/%D1%81%D0%BC%D0%B0%D1%80%D1%82-%D0%BA%D0%BE%D0%BD%D1%82%D1%80%D0%B0%D0%BA%D1%82-%D0%B4%D0%BB%D1%8F-epicchain-769139352b65)
* [Go smart contracts development workshop on YouTube (as a part of the EpicChain Polaris Launchpad)](https://www.youtube.com/watch?v=o38fXiLG7EM)
* [Go smart contracts development workshop on YouTube (as a part of the EpicChain Asia-Pacific Tour)](https://www.youtube.com/watch?v=q_TMlpx1-0M)
* [Go dApp development workshop on YouTube](https://www.youtube.com/live/8zVBIrVQa58)
* [Using EpicChain Blockchain Toolkit](https://medium.com/@neospcc/EpicChainG-adds-support-for-epicchain-blockchain-toolkit-673ea914f661)
* [EpicChain documentation](https://docs.epic-chain.org/)
* [EpicChain github](https://github.com/epicchain-project/neo/)
* [EpicChainG github](https://github.com/nspcc-dev/epicchain-go)
